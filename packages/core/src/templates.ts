...
function newTopicSh(): string {
  return lines([
    ...
    "  WORKING_BRANCH_PREFIX=\"$(node -e 'const fs=require(\"fs\"); const manifest=JSON.parse(fs.readFileSync(process.argv[1],\"utf8\")); const value=String(manifest.git?.workingBranchPrefix ?? \"ai\").trim().replace(/\\/+$/,\"\") || \"ai\"; process.stdout.write(value);' \"$ROOT_DIR/.pgg/project.json\")\"",
    "  RELEASE_BRANCH_PREFIX=\"$(node -e 'const fs=require(\"fs\"); const manifest=JSON.parse(fs.readFileSync(process.argv[1],\"utf8\")); const value=String(manifest.git?.releaseBranchPrefix ?? \"release\").trim().replace(/\\/+$/,\"\") || \"release\"; process.stdout.write(value);' \"$ROOT_DIR/.pgg/project.json\")\"",
