...
function workFlowMd(input: TemplateInput): string {
  if (input.language === "ko") {
    return lines([
      ...
      "3. `pgg-code`",
      "   - 코드 구현",
      "   - `pgg git=on`이면 task 완료마다 `.codex/sh/pgg-stage-commit.sh <topic|topic_dir> implementation <summary> <why> [footer]`로 task-scoped commit을 남기고, helper는 필요 시 governed `working_branch` auto-checkout recovery를 먼저 시도한다",
      "   - `implementation/index.md`, `implementation/diffs/*.diff`, `reviews/code.review.md` 생성",
      "4. `pgg-refactor`",
      "   - 구수행 코드 검사 후 레거시 코드 제거와 구조 개선 수행으로 최적화된 클린코드",
      "   - `pgg git=on`이면 refactor task 완료마다 `.codex/sh/pgg-stage-commit.sh <topic|topic_dir> refactor <summary> <why> [footer]`로 refactor proof commit을 남기고, helper는 필요 시 governed `working_branch` auto-checkout recovery를 먼저 시도한다",
      ...
      "   - archive helper가 `version.json`과 append-only `poggn/version-history.ndjson` ledger를 기록하고, `pgg git=on`이면 필요 시 governed `working_branch` auto-checkout recovery를 거쳐 `ai/<archive_type>/<target-version>-<short-name>`에서 `release/<target-version>-<short-name>`으로 승격하는 git publish helper까지 이어진다",
  ...
  return lines([
    ...
    "3. `pgg-code`",
    "   - implement code",
    "   - when `pgg git=on`, record a task-scoped commit after each completed task with `.codex/sh/pgg-stage-commit.sh <topic|topic_dir> implementation <summary> <why> [footer]`, and let the helper attempt governed `working_branch` auto-checkout recovery when needed",
    "   - create `implementation/index.md`, `implementation/diffs/*.diff`, `reviews/code.review.md`",
    "4. `pgg-refactor`",
    "   - remove legacy code and improve structure",
    "   - when `pgg git=on`, record a refactor-proof commit after each completed refactor task with `.codex/sh/pgg-stage-commit.sh <topic|topic_dir> refactor <summary> <why> [footer]`, and let the helper attempt governed `working_branch` auto-checkout recovery when needed",
    ...
    "   - record `version.json` plus the append-only `poggn/version-history.ndjson` ledger during archive, then continue into git publish that can recover onto the governed `working_branch` before promoting `ai/<archive_type>/<target-version>-<short-name>` into `release/<target-version>-<short-name>` when `pgg git=on`",
