...
function agentsMd(input: TemplateInput): string {
  if (input.language === "ko") {
    return lines([
      ...
      "- `pgg git`이 `on`이면 `state/current.md` 또는 `qa/report.md`의 `Git Publish Message` 섹션으로 commit 제목/Why/footer를 관리하며, 한 커밋 = 하나의 의도, Why 없으면 좋은 커밋 아님, 제목 50자 이하, 명령형 금지, 마침표 금지, 로그가 곧 문서 원칙을 지킨다.",
  ...
  return lines([
    ...
    "- When `pgg git` is `on`, manage commit title, Why body, and footer through the `Git Publish Message` section in `state/current.md` or `qa/report.md`, and keep one commit per intent, no good commit without Why, subjects at 50 characters or fewer, no imperative mood, no period, and logs as documentation.",
    verificationContractRule(input.language),
...
function workFlowMd(input: TemplateInput): string {
  if (input.language === "ko") {
    return lines([
      ...
      "   - `pgg git=on`이면 `state/current.md` 또는 `qa/report.md`의 `## Git Publish Message`에서 `- title:`, `- why:`, `- footer:`를 읽고 제목 50자 이하, 명령형 금지, 마침표 금지, Why 중심 body, 한 커밋 = 하나의 의도 규칙을 검증한다",
      "",
      "### Optional Audits",
      "",
      "- `pgg-token`",
  ...
  return lines([
    ...
    "   - when `pgg git=on`, read `- title:`, `- why:`, and `- footer:` from `## Git Publish Message` in `state/current.md` or `qa/report.md`, then enforce the 50-character, non-imperative, no-period, Why-first, one-commit-per-intent contract",
    "",
    "### Optional Audits",
    "",
    "- `pgg-token`",
...
function stateContractMd(input: TemplateInput): string {
  if (input.language === "ko") {
    return lines([
      ...
      "- 제목은 50자 이하, 명령형 금지, 마침표 금지, 제목만 봐도 의도를 이해할 수 있어야 한다.",
  ...
  return lines([
    ...
    "- Subjects must be 50 characters or fewer, non-imperative, period-free, and understandable on their own.",
  ...
  const korean = {
    ...
    "pgg-qa": {
      ...
      body: [
        ...
        "- `pgg git=on`이면 `qa/report.md` 또는 `state/current.md`에 `## Git Publish Message` 섹션을 두고 `- title:`, `- why:`, `- footer:`를 기록한다",
...
function newTopicSh(): string {
  return lines([
    ...
    "    WORKING_BRANCH=\"ai/$ARCHIVE_TYPE/$TARGET_VERSION-$SHORT_NAME\"",
    "    RELEASE_BRANCH=\"release/$TARGET_VERSION-$SHORT_NAME\"",
...
function versionSh(): string {
  return lines([
    ...
    "EXPECTED_WORKING_BRANCH=\"ai/$ARCHIVE_TYPE/$NEXT_VERSION-$SHORT_NAME\"",
    "EXPECTED_RELEASE_BRANCH=\"release/$NEXT_VERSION-$SHORT_NAME\"",
    "TIMESTAMP=\"$(date -u +\"%Y-%m-%dT%H:%M:%SZ\")\"",
    "",
...
function gitPublishSh(): string {
  return lines([
    ...
    "  COMMIT_TITLE_VALUE=\"$1\" WHY_SUMMARY_VALUE=\"$2\" node - <<'NODE'",
    ...
    "  }",
    "  if (imperativePattern.test(title) || koreanImperativePattern.test(title)) {",
    ...
    "COMMIT_MESSAGE_ERROR=\"$(validate_commit_message \"$COMMIT_TITLE\" \"$WHY_SUMMARY\")\"",
    "",
    "blocked_result() {",
    "  local result_type=\"$1\"",
    "  local reason=\"$2\"",
    ...
    "fi",
    "",
    "declare -a CANDIDATE_PATHS=(\"$ARCHIVE_TOPIC_PATH\" \"$STATE_HISTORY_RELATIVE\" \"$VERSION_LEDGER_RELATIVE\")",
    "declare -a DIRTY_ALLOWED_PATHS=(\"$ARCHIVE_TOPIC_PATH\" \"$STATE_HISTORY_RELATIVE\" \"$VERSION_LEDGER_RELATIVE\" \"$PUBLISH_RELATIVE_PATH\")",
    "",
    ...
    "git -C \"$ROOT_DIR\" add -- \"$PUBLISH_RELATIVE_PATH\" \"$STATE_HISTORY_RELATIVE\"",
    "if ! git -C \"$ROOT_DIR\" diff --cached --quiet; then",
