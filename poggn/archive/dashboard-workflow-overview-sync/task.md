...
| T2 | S2 | `추가 진행` update status를 model에 추가하고 과거 flow에 붙는 historical update 상태를 제거한다. | T1, S2 | 현재 active flow에 `requirements-added` 등 event가 있을 때만 distinct status/accent로 표시할 수 있다 |
...
- 시작 전/생성 중/추가 진행 flow는 small caption으로 상태를 표시한다.
...
- `추가 진행`은 생성 중/완료/시작 전과 다른 색과 badge/pulse로 구분된다.
