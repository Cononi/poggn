...
- completed flow는 trusted `completedAt`, verified/final `stage-completed`, 또는 governed completion commit인 `stage-commit` evidence가 있어야 한다.
- 검증 전 작업 정리용 `stage-completed`는 완료 evidence로 보지 않고 `stage-progress`처럼 진행 중 상태를 유지해야 한다.
...
- verified/final `stage-completed` event 또는 `stage-commit` event가 있는 flow는 해당 timestamp를 완료 시각으로 표시할 수 있다.
