...
- `data.status`: `pending`, `generated`, `revising`, `completed` 또는 dashboard가 매핑 가능한 status
...
- `stage-started`, `stage-progress`, `stage-completed` event가 있으면 Overview Progress에 시작/진행/완료 상태와 시간이 반영된다.
- `proposal-updated` 또는 `stage-revised` event가 있으면 revision status source가 될 수 있다.
