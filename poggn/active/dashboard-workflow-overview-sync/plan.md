...
- 사용자-facing status는 `진행 전`, `생성 중`, `추가 요소 반영 중`, `완료`로 i18n 처리하고 상태별 색/connector/icon/badge를 분리한다.
...
3. S5를 적용해 `진행 전`, `생성 중`, `추가 요소 반영 중`, `완료`, tooltip 문구를 ko/en dictionary로 준비한다.
...
- 추가 요구가 들어온 flow는 `추가 요소 반영 중` 상태와 별도 accent color로 표시 가능해야 한다.
...
- `추가 요소 반영 중`은 새 stage가 아니라 기존 stage의 transient status다.
