...
function HistoryOverview(props: {
  ...
  const active = steps.filter((step) => step.status === "current" || step.status === "next").length;
  const pending = steps.filter((step) => step.status === "pending").length;
  const current = steps.find((step) => step.status === "current") ?? steps.find((step) => step.status === "next") ?? steps[0];
  const completion = Math.round((completed / Math.max(steps.length, 1)) * 100);
  ...
  return (
    <Stack spacing={1.5}>
      ...
      >
        ...
        <OverviewStat title="Workflow Stage" value={current?.label ?? "Add"} helper={`${Math.min(completed + active, steps.length)} of ${steps.length} steps`} tone="primary" />
        ...
        <Stack spacing={2}>
          ...
          >
            ...
            <Stack direction="row" spacing={2} sx={{ alignItems: "center", justifyContent: "center", minWidth: 0 }}>
              <WorkflowProgressChart completed={completed} active={active} pending={pending} completion={completion} />
              <Stack spacing={1}>
                <LegendDot color={theme.palette.success.main} label="Completed" value={completed} />
                <LegendDot color={theme.palette.primary.main} label="In Progress" value={active} />
...
function WorkflowStepNode(props: { step: WorkflowStep; first: boolean; onSelect: () => void }) {
  ...
  const color =
    props.step.status === "completed"
      ? theme.palette.success.main
      : props.step.status === "current" || props.step.status === "next"
        ? theme.palette.primary.main
        : theme.palette.text.secondary;
